...
int main(int argc,char* argv[])
{
  ...
  for(L=0; L<9; L++){
    for(C=0; C<9; C++){
      ...
      printf("MAIN %p) Lancement de threadCase\n", pthread_self()); fflush(stdout);
      if(pthread_create(&tabThreadCase[L][C],NULL,FctThreadCase,c)!=0){
        printf("Erreur création threadCase\n");
        exit(1);
      }
    }
  }
...
void *FctThreadScore(void *p){
  ...
  while(1){
    ...
    while(!MAJScore && !MAJCombos){
      ...
      printf("ThreadScore (%p) : notification reçue (score=%d)\n", pthread_self(), score);
    }
    val=score;
    comb=combos;
    MAJScore=false;
    ...
    DessineChiffre(1,17, val%10);
    val=val/10;
    DessineChiffre(1,16, val%10);
    val=val/10;
    DessineChiffre(1,15, val%10);
    val=val/10;
    DessineChiffre(1,14, val%10);

    DessineChiffre(8,17, comb%10);
    comb=comb/10;
    DessineChiffre(8,16, comb%10);
    comb=comb/10;
    DessineChiffre(8,15, comb%10);
    comb=comb/10;
    DessineChiffre(8,14, comb%10);
...
void handlerSIGUSR1(int s){
  ...
  c=(CASE*)pthread_getspecific(cleCase);
  L=c->ligne;
  C=c->colonne;
